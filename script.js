...
const translations = {
  pt: {
    ...
    pdfHref: "assets/cv-rafael-rino-pt-pt.pdf",
    pdfFileName: "cv-rafael-rino-pt-pt.pdf",
    ...
    profileTitle: "Perfil",
    profileParagraphs: [
      "Profissional de 30 anos, natural de Leiria, com experiência como Operador Tático da Polícia Aérea em missões de segurança, proteção de infraestruturas, exercícios conjuntos e operações no âmbito das Forças Armadas Portuguesas. Traz uma base sólida de disciplina, responsabilidade, trabalho sob pressão e cumprimento rigoroso de procedimentos.",
    ...
    educationCards: [
      ...
      {
        title: "12.º ano · Ciências e Tecnologias",
        text: "Ensino secundário concluído na área de Ciências e Tecnologias."
      },
      {
    ...
    enterpriseProjects: [
      ...
      {
        kicker: "SOC · DRTMoldes · NIS2",
        title: "Laboratório Virtual SOC para Implementação Empresarial",
        items: [
          "Desenvolvimento de um laboratório SOC em ambiente doméstico para avaliar a viabilidade de futura implementação interna na DRTMoldes, alinhada com requisitos NIS2.",
          "Implementação do Wazuh como SIEM principal para centralização de logs, monitorização de endpoints Linux, Syscheck, eventos de autenticação e recolha de eventos do sistema operativo.",
          "Instalação e configuração do Suricata como NIDS, incluindo ativação de output JSON em eve.json, integração com Wazuh e atualização de regras ET Open através de suricata-update.",
          "Validação da infraestrutura com scans Nmap, alterações em ficheiros monitorizados e tentativas de autenticação falhadas para testar deteção, correlação e alertas.",
          "Implementação do TheHive para gestão de incidentes e criação de integração com Wazuh através de script Python e API REST para envio de alertas e abertura de casos.",
          "Implementação do Shuffle como plataforma SOAR em Docker, usando webhooks e API Keys para automação e resposta a incidentes entre ferramentas de segurança.",
          "Resolução de problemas técnicos relacionados com regras Suricata, integração entre plataformas, containers Docker, conflitos de portas OpenSearch e troubleshooting em Linux."
        ]
      },
      {
        kicker: "fancup.net · AWS · Cloudflare",
        title: "fancup.net cloud infrastructure configuration",
        items: [
          "Configuração da infraestrutura AWS do fancup.net com servidores Windows Server 2022 para APP e base de dados, IIS para alojamento da aplicação, SQL Server, Elastic IP e Cloudflare.",
          "Configuração e validação de CloudWatch Agent, métricas de CPU/RAM/disco, logs IIS, alarmes APP/DB, SNS e dashboard FUNCUP-INFRA.",
          "Configuração de AWS Backup, criação de AMI golden, recuperação de EC2, failover, Load Balancer, Launch Template e Auto Scaling.",
          "Configuração de HTTPS com Cloudflare Full (Strict), certificado Origin, binding IIS 443, redirect HTTP para HTTPS e HSTS.",
          "Integração e validação de Ollama local com modelo Gemma para componente de IA, com ChatProvider, endpoint local e testes funcionais."
        ]
      }
    ],
  ...
  en: {
    ...
    pdfHref: "assets/cv-rafael-rino-en.pdf",
    pdfFileName: "cv-rafael-rino-en.pdf",
    ...
    educationCards: [
      ...
      {
        title: "12th grade · Science and Technology",
        text: "Secondary education completed in Science and Technology."
      },
      {
    ...
    enterpriseProjects: [
      ...
      {
        kicker: "SOC · DRTMoldes · NIS2",
        title: "Virtual SOC Lab for Business Implementation",
        items: [
          "Development of a home-based SOC lab to assess the feasibility of a future internal implementation at DRTMoldes aligned with NIS2 requirements.",
          "Implementation of Wazuh as the main SIEM for log centralization, Linux endpoint monitoring, Syscheck, authentication events and operating system event collection.",
          "Installation and configuration of Suricata as a NIDS, including JSON output in eve.json, Wazuh integration and ET Open rule updates through suricata-update.",
          "Validation of the infrastructure with Nmap scans, monitored file changes and failed authentication attempts to test detection, correlation and alerts.",
          "Implementation of TheHive for incident management and creation of a Wazuh integration through a Python script and REST API to send alerts and open cases.",
          "Implementation of Shuffle as a SOAR platform in Docker, using webhooks and API keys for automation and incident response across security tools.",
          "Troubleshooting of Suricata rules, platform integrations, Docker containers, OpenSearch port conflicts and Linux service issues."
        ]
      },
      {
        kicker: "fancup.net · AWS · Cloudflare",
        title: "fancup.net cloud infrastructure configuration",
        items: [
          "Configuration of the fancup.net AWS infrastructure with Windows Server 2022 servers for the application and database, IIS hosting the application, SQL Server, Elastic IP and Cloudflare.",
          "Configuration and validation of CloudWatch Agent, CPU/RAM/disk metrics, IIS logs, APP/DB alarms, SNS and the FUNCUP-INFRA dashboard.",
          "Configuration of AWS Backup, golden AMI creation, EC2 recovery, failover, Load Balancer, Launch Template and Auto Scaling.",
          "Configuration of HTTPS with Cloudflare Full (Strict), Origin Certificate, IIS 443 binding, HTTP to HTTPS redirect and HSTS.",
          "Integration and validation of local Ollama with the Gemma model for the AI component, with ChatProvider, local endpoint and functional tests."
        ]
      }
    ],
...
function applyLanguage(language) {
  ...
  document.querySelectorAll(".hero-actions .button.secondary, #contacto .button.secondary").forEach((link) => {
    link.setAttribute("href", t.pdfHref);
    link.setAttribute("download", t.pdfFileName);
  });
  ...
  if (educationCards[1]) {
    ...
    educationCards[1].querySelector("p").textContent = t.educationCards[1].text;
  }
  if (educationCards[2]) {
    educationCards[2].querySelector("h3").textContent = t.educationCards[2].title;
    educationCards[2].querySelectorAll("li").forEach((item, index) => {
      if (t.educationCards[2].items[index] !== undefined) item.textContent = t.educationCards[2].items[index];
    });
  }
